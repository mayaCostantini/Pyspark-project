...
CPU_request = task_events_entries.map(lambda x: (x[machine_index] + ',' + x[task_index], x[CPU_request_index]))


# RDD with ('task index', 'sampled CPU usage') : 
CPU_usage = task_usage_entries.map(lambda x: (x[machine_index] + ',' + x[task_index], x[sampled_CPU_usage_index]))

union = sc.union 

# RDD with ('task index', ['CPU request', 'sampled CPU usage'])
CPU_request_vs_usage = CPU_request.union(CPU_usage).reduceByKey(lambda a,b: a + ',' + b).filter(lambda x: len(x[1].split(',')) == 2)
